...
int main(void) {
  ...
  errno = 0;
  assert(raplcap_is_zone_supported(0, NULL, RAPLCAP_ZONE_PACKAGE) < 0);
  assert(errno == EINVAL);
  errno = 0;
  assert(raplcap_is_zone_enabled(0, NULL, RAPLCAP_ZONE_PACKAGE) < 0);
  assert(errno == EINVAL);
  errno = 0;
  assert(raplcap_set_zone_enabled(0, NULL, RAPLCAP_ZONE_PACKAGE, 0) < 0);
  assert(errno == EINVAL);
  errno = 0;
  assert(raplcap_get_limits(0, NULL, RAPLCAP_ZONE_PACKAGE, NULL, NULL) < 0);
  assert(errno == EINVAL);
  errno = 0;
  assert(raplcap_set_limits(0, NULL, RAPLCAP_ZONE_PACKAGE, NULL, NULL) < 0);
  assert(errno == EINVAL);

...
int raplcap_init(raplcap* rc) {
  ...
  if ((state->sys = msr_sys_init(&rc->nsockets)) == NULL) {
    free(state);
    return -1;
  }
  rc->state = state;
  if (msr_sys_read(state->sys, &msrval, 0, MSR_RAPL_POWER_UNIT)) {
    err_save = errno;
    raplcap_destroy(rc);
    errno = err_save;
    return -1;
...
int raplcap_is_zone_enabled(const raplcap* rc, uint32_t socket, raplcap_zone zone) {
  ...
  if (state == NULL || msr < 0 || msr_sys_read(state->sys, &msrval, socket, msr)) {
    return -1;
  }
...
int raplcap_is_zone_supported(const raplcap* rc, uint32_t socket, raplcap_zone zone) {
  ...
  ret = msr_sys_read(state->sys, &msrval, socket, msr) ? 0 : 1;
...
int raplcap_set_limits(const raplcap* rc, uint32_t socket, raplcap_zone zone,
  ...
  return msr_sys_write(state->sys, msrval, socket, msr);
}
